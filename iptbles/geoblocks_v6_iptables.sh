...
# GeoBlock IPv6 ipset Generator
# Downloads MaxMind GeoLite2-Country CSV, extracts IPv6 networks for blocked
# countries, and writes shell scripts to apply/update an ipset on Linux.
#
# Wire up once (run manually or in a bootstrap script):
#   ipset create GeoBlock6 hash:net family inet6 maxelem 200000
#   ip6tables -I INPUT   -m set --match-set GeoBlock6 src -j DROP
#   ip6tables -I FORWARD -m set --match-set GeoBlock6 src -j DROP
#
# Usage: ./geoblocks_v6_iptables.sh [--dry-run]
...
FULL_SCRIPT="$OUTPUT_DIR/geoblock6.sh"
UPDATE_SCRIPT="$OUTPUT_DIR/geoblock6_update.sh"
...
{
    echo "#!/bin/bash"
    echo "# GeoBlock IPv6 full ipset reset — generated $TS"
    echo "# Wire up once if not already done:"
    echo "#   ipset create $ADDRESS_LIST_NAME hash:net family inet6 maxelem $IPSET_MAXELEM"
    echo "#   ip6tables -I INPUT   -m set --match-set $ADDRESS_LIST_NAME src -j DROP"
    echo "#   ip6tables -I FORWARD -m set --match-set $ADDRESS_LIST_NAME src -j DROP"
    echo "set -euo pipefail"
    echo ""
    echo "ipset create $ADDRESS_LIST_NAME hash:net family inet6 maxelem $IPSET_MAXELEM 2>/dev/null \\"
    echo "  || ipset flush $ADDRESS_LIST_NAME"
    echo ""
...
{
    echo "#!/bin/bash"
    echo "# GeoBlock IPv6 incremental ipset update — generated $TS"
...
send_discord "GeoBlock6 (ipset) updated. Networks: $TOTAL_NETWORKS | Added: $ADDED_COUNT, Removed: $REMOVED_COUNT | Full: ${FULL_KB}KB/${FULL_LINES} lines | Update: ${UPDATE_KB}KB/${UPDATE_LINES} lines"
log "Done."
