...
# GeoBlock List Generator
# Generates MikroTik address-list for blocked country IP ranges.
#
# Usage: ./geoblocks.sh [--dry-run]
...
FULL_SCRIPT="$OUTPUT_DIR/geoblock.rsc"
UPDATE_SCRIPT="$OUTPUT_DIR/geoblock_update.rsc"
...
# ------------------------------------------------------------------------------
# Extract networks for each country
# ------------------------------------------------------------------------------
...

{
    ...
    while IFS=',' read -r network _; do
        echo "/ip firewall address-list remove [find list=$ADDRESS_LIST_NAME address=$network]"
...
send_discord "GeoBlock updated. Networks: $TOTAL_NETWORKS | Added: $ADDED_COUNT, Removed: $REMOVED_COUNT | Full: ${FULL_KB}KB/${FULL_LINES} lines | Update: ${UPDATE_KB}KB/${UPDATE_LINES} lines"
log "Done."
